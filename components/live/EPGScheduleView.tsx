...
export function EPGScheduleView({ epgItems }: EPGScheduleViewProps) {
    ...
    return (
        <div className="space-y-4">
            <h2 className="text-lg font-bold text-white mb-4">{t("tvSchedule")}</h2>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="w-[2px] h-10 bg-white/10 shrink-0 self-stretch relative">
                                {isCurrent && <div className="absolute top-0 bottom-0 left-[-1px] right-[-1px] bg-[color:var(--accent)] w-[4px] rounded-full" />}
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className={`text-sm font-bold truncate ${isCurrent ? "text-[color:var(--accent)]" : "text-white"}`}>
                                    {item.program_title}
                                </h3>
                                <p className="text-xs text-white/50 truncate">
                                    {item.program_desc || t("noDescription")}
                                </p>
                            </div>
