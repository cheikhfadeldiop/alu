    ...
    React.useEffect(() => {
        ...
        async function fetchRelated() {
            ...
            try {
                ...
                setRelatedVideos(items.filter(i => i.slug !== video.slug));
            } catch (err) {
    ...
    return (
        <div ref={containerRef} className=" rounded-sm w-full overflow-hidden group/player mb-12">
            <div className="flex flex-col lg:flex-row h-auto lg:h-[720px]">
                    ...
                    {!youtubeId && (
                        <div className="h-28 bg-[#0a0a0a] border-t border-white/5 flex flex-col justify-center px-10 relative group/controls">
                            ...
                            <div className="grid grid-cols-3 items-center w-full mt-4">
                                <div className="flex items-center gap-4"><button onClick={toggleMute} className="text-white/40 hover:text-white">{isMuted ? <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg> : <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>}</button><input type="range" min={0} max={1} step={0.05} value={volume} onChange={handleVolumeChange} className="w-24 accent-red-600" /></div>
                                <div className="flex justify-center"><button onClick={togglePlay} className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center hover:border-red-600 border border-white/10 transition-all">{isPlaying ? <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> : <svg className="w-6 h-6 text-white ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>}</button></div>
                                <div className="flex items-center justify-end gap-6">
                                    <button onClick={() => setShowQualityMenu(!showQualityMenu)} className="text-[10px] font-black text-white/40 border border-white/10 px-3 py-1 rounded-lg uppercase">{currentQuality === -1 ? 'Auto' : `${qualities.find(q => q.index === currentQuality)?.height}P`}</button>
                                    <button onClick={toggleFullscreen} className="text-white/40 hover:text-red-600"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 4l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg></button>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                {/* Right: Sidebar */}
                <div ref={sidebarRef} onScroll={handleSidebarScroll} className="w-full lg:w-1/4 bg-[#0a0a0a] flex flex-col h-full shadow-2xl relative">
                    <div className="p-8 border-b border-white/5 bg-black/40"><h3 className="text-base font-black text-white uppercase">Émissions Précédentes</h3></div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-5 scrollbar-thin scrollbar-thumb-red-600">
                        {loadingRelated ? (
                            <div className="animate-pulse space-y-8">{[1, 2, 3].map(i => <div key={i} className="aspect-video bg-white/5 rounded-3xl" />)}</div>
                        ) : relatedVideos.map((item, idx) => (
                            <Link href={`/replay/${item.slug}`} key={`${item.slug}-${idx}`} scroll={false} className="group block p-2 rounded-[2rem] hover:bg-white/5 transition-all">
                                <div className="relative aspect-video rounded-2xl overflow-hidden mb-4 bg-zinc-900 border border-white/5">
                                    <SafeImage src={item.logo_url || item.logo} alt={item.title} fill className="object-cover group-hover:scale-110 transition-transform duration-700 opacity-70 group-hover:opacity-100" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent opacity-60" />
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center"><svg className="w-5 h-5 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg></div></div>
                                </div>
                                <h4 className="text-[10px] font-black uppercase text-white/50 group-hover:text-white line-clamp-2 text-center">{item.title}</h4>
                            </Link>
                        ))}
                    </div>
                </div>
