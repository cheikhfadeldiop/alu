            ...
            {video ? (
                ...
                <div className="w-full aspect-video bg-[#0a0a0a] flex flex-col items-center justify-center gap-6 border border-white/5 shadow-2xl overflow-hidden">
                    ...
                    <div className="space-y-2 text-center">
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[color:var(--accent)] animate-pulse block">{t("deepSearch")}</span>
                        <p className="text-[9px] text-white/40 uppercase font-bold tracking-widest">{t("fetchingArchive")}</p>
                    </div>
                </div>
            ) : (
                <div className="w-full aspect-video bg-[#0a0a0a] flex flex-col items-center justify-center gap-6 border border-white/5 shadow-2xl overflow-hidden text-center px-10">
                    <div className="w-20 h-20 rounded-full bg-[color:var(--accent)]/5 flex items-center justify-center border border-[color:var(--accent)]/10">
                        <svg className="w-10 h-10 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                    </div>
                    <div className="space-y-2">
                        <h3 className="text-lg font-black text-white uppercase tracking-tighter">{t("episodeNotFound")}</h3>
                        <p className="text-xs text-white/40 max-w-xs mx-auto leading-relaxed">
                            {t("contentUnavailable")}
                        </p>
                    </div>
                    <button onClick={() => window.location.reload()} className="px-8 py-3 bg-white/5 hover:bg-white/10 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all text-white border border-white/10">{t("retrySearch")}</button>
                </div>
            )}
        </div>
    );
